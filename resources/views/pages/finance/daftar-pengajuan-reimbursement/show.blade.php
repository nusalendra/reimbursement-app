    ...
    <div class="py-12">
        <div class="max-w-5xl rounded-lg mx-auto sm:px-6 lg:px-8 bg-white shadow-md mb-4">
            <div class="max-w-4xl mx-auto py-6">
                ...
                <div class="flex">
                    <form id="reimbursementForm" action="/daftar-pengajuan-reimbursement/{{ $data->id }}"
                        method="POST">
                        @csrf
                        @method('put')
                        ...
                        <button type="button" onclick="handlePengajuan('{{ $data->id }}', 'Ditolak')"
                            ...
                            Pengajuan</button>
                        <button type="button" onclick="handlePengajuan('{{ $data->id }}', 'Diterima')"
                            value="Diterima" name="terima_pengajuan"
                            class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Terima Pengajuan</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        async function handlePengajuan(id, status) {
            if (status === 'Ditolak') {
                ...
                document.getElementById("status_pengajuan").value = 'Diterima';
                document.getElementById("reimbursementForm").submit();
            }
        }
    </script>
